function [mean_dist, std_dist, max_dist, ix, iy] = dtw_quat_oneWay(s, t)
    ns = size(s, 1);
    nt = size(t, 1);
    cost = inf(ns, nt);
    distance_matrix = zeros(ns, nt); % To store distances for statistical analysis

    % Initialize cost matrix and distance matrix
    cost(1,1) = distOrientation(s(1,:), t(1,:));
    distance_matrix(1,1) = cost(1,1);
    for i = 2:ns
        distance_matrix(i,1) = distOrientation(s(i,:), t(1,:));
        cost(i,1) = cost(i-1,1) + distance_matrix(i,1);
    end
    for j = 2:nt
        distance_matrix(1,j) = distOrientation(s(1,:), t(j,:));
    ...
    for i = 2:ns
        for j = 2:nt
            ...
            localCost = distOrientation(s(i,:), t(j,:));
